...
Three chip groups:
- *Layers:* `[Nature]` `[Lodging]` `[Airport + Jewish]` — toggle whole layer on/off.
- *Regions:* `[Salzkammergut]` `[Berchtesgaden]` `[Hohe Tauern]` — filters nature pins within the Nature layer.
- *Toggles:* `[Show 7-day route]` `[Show drive lines]` — extras.
...
- Right-rail panel, 280px wide. Sits *beside* the map (flex layout), not over it.
...
- Route polyline gets a subtle scroll-into-view draw animation via IntersectionObserver + CSS `stroke-dashoffset` transition. Already a dashed line — we make the dash *appear* over ~1.4s the first time the section enters viewport. Subsequent scrolls = no re-animation.
...
- Eleken — *Map UI Design: Best Practices, Tools & Real-World Examples* — visual hierarchy + popup-vs-sidebar tradeoffs.
- Felt blog — *How to make interactive maps* — reveal-on-demand tooltip pattern.
...
- Airbnb Tech Blog — *Improving search ranking for Maps* (mini-pin vs price-pin hierarchy).
